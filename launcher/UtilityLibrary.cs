...
namespace ROF_Downloader
{
    ...
    class UtilityLibrary
    {
        ...
        public static async Task<bool> CreatePEQDB(int startProgress, int endProgress)
        {
            try
            {
                StatusLibrary.SetScope("Create PEQ DB query");
                ...
                StatusLibrary.SetScope("Read db exists");
                ...
                if (!isDBCreated)
                {
                    StatusLibrary.SetScope("Create DB");
                ...
                StatusLibrary.SetScope("Verify tables");
                ...
                StatusLibrary.SetScope("Read Tables");
                while (await reader.ReadAsync(StatusLibrary.CancelToken()))
                {
                    string line = reader.GetString(0);
            ...
            {
                StatusLibrary.LogEvent($"Failed to CreatePEQDB: {ex.Message}");
            }
            return true;
        }
        ...
        {
            StatusLibrary.SetScope("Sourcing database");
            try
            {
                StatusLibrary.SetProgress(startProgress);
                StatusLibrary.SetStatusBar("Sourcing database...");
                StatusLibrary.SetScope("Running peq_download");
                ...
                proc.ErrorDataReceived += new DataReceivedEventHandler((object sender, DataReceivedEventArgs e) =>
                {
                    ...
                    if (!hasErrorOccured)
                    {
                        StatusLibrary.LogEvent($"Error reported during peq_download: {line}");
                    }
