...
namespace ROF_Downloader
{
    internal class Server
    {
        ...
        public static async Task Fix(bool fixAll)
        {
            try
            {
                StatusLibrary.SetEvent("Server Fix");
                if (SQL.IsRunning())
                {
                    var result = MessageBox.Show("SQL is currently running, Fippy needs to shut it down to download latest PEQ, is this OK?", "Stop SQL", MessageBoxButtons.OKCancel, MessageBoxIcon.Information);
                    if (result == DialogResult.Cancel)
                    {
                        StatusLibrary.SetStatusBar("Cancelled, another SQL instance running and opted not to stop it");
                        return;
                    }
                    SQL.Stop();
                }
                ...
                if (Directory.Exists(path))
                {
                    StatusLibrary.SetEvent("Verifying Old SQL Connection Settings");
                    SQL.Start();
                    if (!SQL.VerifySettings())
                    {
                        StatusLibrary.Log("Prompting user if they want to delete data, since we can't access it");
                        var result = MessageBox.Show("Fippy can't connect to SQL!\nI tried the blank default root password, and the value inside eqemu_config, but both failed.\nDo you want me to delete the SQL folder and start from scratch?", "SQL Failed", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                        if (result == DialogResult.No)
                        {
                            StatusLibrary.LogEvent("Failed to connect to SQL, and don't want to delete data");
                            return;
                        }
                        ...
                        {                            
                            ...
                            StatusLibrary.Log("Deleting old data path");
                        }
                    }
                }
                ...
                if (!Directory.Exists(path))
                {
                    ...
                    StatusLibrary.SetEvent("Running mysql_install_db to create data directory");
                    ...
                    proc.ErrorDataReceived += new DataReceivedEventHandler((object src, DataReceivedEventArgs earg) =>
                    {
                        ...
                        if (!hasErrorOccured)
                        {
                            StatusLibrary.LogEvent(line);
                        }
                        hasErrorOccured = true;
                        
                        
                        StatusLibrary.Log($"Sql Install Error: {line}");
                    });
                ...
                if (isNewConfig)
                {
                    ...
                    try
                    {
                        StatusLibrary.SetEvent("Setting root password");
                        StatusLibrary.SetScope("Mapping config");
                        ...
                        StatusLibrary.SetScope("Writing file");
                        ...
                        StatusLibrary.SetScope("Running root inject");
                        ...
                        proc.ErrorDataReceived += new DataReceivedEventHandler((object src, DataReceivedEventArgs earg) =>
                        {
                            ...
                            if (!hasError)
                            {
                                StatusLibrary.LogEvent("Failed to set root password response: ${line}");
                            }
                            hasError = true;
                            StatusLibrary.Log($"Sql Fix Error: {line}");
                        });
                        proc.Start();
                        proc.BeginErrorReadLine();
                        proc.BeginOutputReadLine();
                ...
                stage = await UtilityLibrary.Download(32, 33, "https://raw.githubusercontent.com/xackery/downloadeqrof/main/assets/portable_settings.txt", "db\\heidi", "portable_settings.txt", 1);
                if (stage == -1) { return; }
                if (!fixAll && stage > startStage) { return; }
                ...
                {
                    StatusLibrary.SetScope($"Create {path}");
                    StatusLibrary.LogEvent(ex.Message);
                }
                ...
                stage = await UtilityLibrary.Download(75, 76, "https://raw.githubusercontent.com/xackery/downloadeqrof/main/assets/peq_download.pl", "server", "peq_download.pl", 30);
                if (stage == -1) { return; }
                if (!fixAll && stage > startStage) { return; }

                isNewDB = await UtilityLibrary.CreatePEQDB(76, 77);
                ...
                {
                    StatusLibrary.SetScope($"Failed to delete extracted cache peq-db-latest");
                    StatusLibrary.LogEvent(ex.Message);
                }
                ...
                {
                    StatusLibrary.SetScope($"Failed to delete root password reset file in cache");
                    StatusLibrary.LogEvent(ex.Message);
                }
            ...
            } catch (Exception ex)
            {
                ...
                StatusLibrary.LogEvent($"General fallback error: {ex.Message}");
